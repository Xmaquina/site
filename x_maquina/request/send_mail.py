...
def send_email(receiver, sender, subject, mail_body, server_loged):
    header = 'To:' + receiver + '\n' + 'From: ' + sender + '\n' + 'Subject:'+ subject + '\n'
    mail = header + mail_body
    server_loged.sendmail(sender, receiver, mail)

def send_message(user_email, user_password, receiver, subject, mail_body):
    server = connect_server()
    do_login(user_email, user_password, server)
    send_email(receiver, user_email, subject, mail_body, server)
    server.close()
